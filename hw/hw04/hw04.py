...
class interval:
    ...
    def __mul__(self, other):
        fir = self._low * other._low
        sec = self._high * other._high
        thir = self._low * other._high
        fou = self._high * other._low
        lst = [fir, sec, thir, fou]
    ...
    def __truediv__(self, other):
        if (other._low < 0 and other._high > 0) or other._low == 0 or other._high == 0:
            ...
            return interval(
                min(self._low / other._high, self._high / other._low, self._low / other._low, self._high / other._high),
                max(self._low / other._high, self._high / other._low, self._low / other._low, self._high / other._high))
...
class Link:
    ...
    def __add__(self, lst):
        ...
        """
        # self is not empty list

        ret = Link(self.first)
        tmp = ret
        iterator = self.rest
        while iterator != ():
            tmp.rest = Link(iterator.first)
            iterator = iterator.rest
            tmp = tmp.rest
...
def same_shape(t1, t2):
    ...
    for i in range(0, len(t1.children)):
        if same_shape(t1.children[i], t2.children[i]) == False:
            return False
    return True
...
def dfs(tree, linklist, final):
    tmp = add(linklist, Link(tree.label))
    if tree.children == []:
        final.append(tmp)
...
def long_paths(tree, n):
    ...
    res = []
    for item in final:
        if len(item) > n and item not in res:
            res.append(item)
    return res
...
print(long_paths(whole, 4))

def is_odd(n):
    return n % 2 == 1
#map reduce filter返回的是iterable obj
