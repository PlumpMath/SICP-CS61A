...
def group_by_centroid(restaurants, centroids):
    ...
    for item in restaurants:
        loc = restaurant_location(item)# 得到每一个餐馆的位置
        closest = find_closest(loc, centroids)
        dicter[tuple(closest)].append(item)
    res = []
    for item in dicter:
        #这里需要检测是不是为空的
        if dicter[item]:
            res.append(dicter[item])
    return res
...
def find_centroid(cluster):
    ...
    # BEGIN Question 5
    listx = [restaurant_location(item)[0] for item in cluster]
    listy = [restaurant_location(item)[1] for item in cluster]

    return [sum(listx)/len(listx), sum(listy)/len(listy)]
    # END Question 5
...
def best_predictor(user, restaurants, feature_fns):
    ...
    """
    user_reviewed_restaurants_by_user = user_reviewed_restaurants(user, restaurants)
    # BEGIN Question 8
    return max([find_predictor(user, user_reviewed_restaurants_by_user, feature)[0] for feature in feature_fns], key=lambda x : find_predictor(user, user_reviewed_restaurants_by_user, x)[1])
    # END Question 8
