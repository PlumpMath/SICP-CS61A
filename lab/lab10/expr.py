...
class Name(Expr):
    ...
    def eval(self, env):
        ...
        if self.string not in env:
            raise NameError('Not in this environment...Not defined')
        else:
...
class CallExpr(Expr):
    ...
    def eval(self, env):
        ...
        Number(14)
        """
        # 先判断他们是不是在当前的环境中，如果不在，抛出异常
        if self.operator not in env:
            raise NameError('Not in this environment...Not defined')
        ...
        lister = [item.eval(env) for item in self.operands]
        return Number(fn(lister))
